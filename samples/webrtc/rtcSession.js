...
function RtcSession(rtcClient){
	...
	session.pc.onicecandidate = function(e) {
		if (e.candidate) {
			...
			session.pc.addStream(session.client.selfstream);
		}
	};
	...
	session.enableSelfVideo(session.client.options.mediaContraints);
}
...
RtcSession.prototype = {
	...
	enableSelfVideo:function(mediaContraints){
		var session = this;
		if (typeof(session.client.selfstream) == 'undefined'){
			navigator.getUserMedia(mediaContraints, function(stream) {
				...
				$('#' + session.client.options.localVideo)[0].srcObject = session.client.selfstream;
	...
	onCallRequest:function(d){
		...
		$.messager.confirm('call from ' + d.userId,'request a ' + requestInfo.type + ' call',function(e){
	...
	onRtcCandidate:function(d) {
		var candidate = new RTCIceCandidate(d.data);
