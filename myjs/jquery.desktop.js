$(function () {
	...
	function initLayout(target) {
		...
		var center = $('<div/>').attr({
				'border' : false,
				'region' : 'center'
			}).addClass('desktop-wall').appendTo(jqTarget);
		
		//为桌面绑定右键菜
		center.bind('contextmenu', function (e) {
			var appContainer = $.data(target, 'desktop')['appContainer'];
		...
		$.data(target, 'desktop')['calendarDiv'] = calendarDiv;
	...
	function initTaskBlank(target) {
		...
		var taskBar = $('<div/>').addClass('desktop-taskBar'); //创建任务栏对象
		...
		if ($.browser.msie) { //兼容ie的hover
			start.hover(function () {
				$(this).addClass('desktop-startMenu-hover');
			}, function () {
				$(this).removeClass('desktop-startMenu-hover');
			});
		}
		
		if (opts.taskBlankPos == 'south' || opts.taskBlankPos == 'north') {
			taskBar.addClass('desktop-taskBar-bg1');
			start.addClass('desktop-startMenu-x');
			taskList.addClass('desktop-taskList-x');
			calendar.addClass('desktop-taskBar-calendar-x');
			
			var scrollLeft = $('<div/>').addClass('desktop-scroll-left').appendTo(taskList);
			var scrollRight = $('<div/>').addClass('desktop-scroll-right').appendTo(taskList);
			var listWrap = $('<div/>').addClass('desktop-list-wrap').appendTo(taskList);
			var list = $('<ul/>').addClass('desktop-list-list').appendTo(listWrap);
			
		} else {
			taskBar.addClass('desktop-taskBar-bg2');
			start.addClass('desktop-startMenu-y');
			taskList.addClass('desktop-taskList-y');
			calendar.addClass('desktop-taskBar-calendar-y');
			
			var scrollTop = $('<div/>').addClass('desktop-scroll-top').appendTo(taskList);
			var scrollBottom = $('<div/>').addClass('desktop-scroll-bottom').appendTo(taskList);
		}
		...
		$.data(target, 'desktop')['taskBar'] = taskBar;
		$.data(target, 'desktop')['start'] = start;
		$.data(target, 'desktop')['taskList'] = taskList;
		$.data(target, 'desktop')['calendar'] = calendar;
	...
	function setTaskListWidth(target) {
		...
		var taskList = $.data(target, 'desktop')['taskList'];
	...
	function initDeskTop(target) {
		...
		$.data(target, 'desktop')['appContainer'] = appContainer;
		...
		if (opts.loadUrl.app && !loaded) {
			$.ajax({
				...
				success : function (resp) {
					console.log("initDeskTop():url return",opts.loadUrl.app,resp,target);
					initApp(target, resp);
				},
	...
	function initApp(target, apps) {
		var jqTarget = $(target);
		console.log("initApp():target=",target);
		var opts = $.data(target, 'desktop').options;
		var wall = jqTarget.layout('panel', 'center'); //桌面对象
		var appContainer = jqTarget.data().desktop.appContainer;
		...
		for (var i in apps) {
			...
			appItem.data('app', app); //绑定每个app的详细信息到desktop元素上
			...
			if (opts.dbClick) { //绑定App的点击事件（dbClick是否双击）
				appItem.on('dblclick', function () {
					console.log('dblClick():obj=',this,"target=",target)
	...
	function refresh(target, href) {
		...
		if (href) {
			$.ajax({
				...
				cache : false,
				success : function (resp) {
					jqTarget.data().desktop.appContainer.empty();
					initApp(target, resp);
				},
				error : function (XMLHttpRequest, textStatus, errorThrown) {
					$.messager.alert("", textStatus || errorThrown, "error");
	...
	function initMenu(target, menus) {
		...
		var start = $.data(target, 'desktop')['start'];
		//确定菜单显示位置
		var left = 0,
		top = 0;
	...
	function setCalendarTopAndLeft(target) {
		var calendarDiv = $.data(target, 'desktop')['calendarDiv'];
		var opts = $.data(target, 'desktop').options;
		var css = {};
	...
	function initCalendar(target) {
		var jqTarget = $(target);
		var opts = $.data(target, 'desktop').options;
		var calendar = $.data(target, 'desktop')['calendar'];
		...
		jqTarget.click(function (e) {
			...
			if (c != 'desktop-taskBar-calendar-x' && c != 'desktop-taskBar-calendar-y' && !$.contains(calendarDiv[0], e.target)) {
				calendarDiv.hide();
			}
		});
	...
	function initWidget(target) {
		var jqTarget = $(target);
		var opts = $(target).data('desktop').options;
		var wall = jqTarget.layout('panel', 'center'); //桌面对象
		...
		function init(resp) {
			$.each(resp, function () {
				var w = $("<div/>").addClass("desktop-widget").css(css);
	...
	function openApp(target, options) {
		console.log("openApp()",target,options);
		var jqTarget = $(target),
		opt = $(target).data('desktop').options,
		...
		if (options) {
			...
			console.log("openApp(),uuid=",uuid,"appOpt=",appOpt,"this=",this);
		}
		...
		appWindow.prev('div.window-header').click(function (e) {
			var taskList = jqTarget.data('desktop').taskList;
			var list = taskList.find('ul.desktop-list-list');
		...
		function appendToList(uuid, text) {
			...
			if ($('li[l_id="' + uuid + '"]', list).length) {
				...
				if (wrap.width() > taskList.width()) {
					...
					$('div[class^="desktop-scroll-"]', taskList).show();
				}
	...
	function setWallpaper(target, url) {
		...
		$.data(target, 'desktop').options.wallpaper = url;
	}
	...
	$.fn.desktop = function (options, params) {
		if (typeof options === 'string') {
			return $.fn.desktop.methods[options.toLowerCase()].call(this, params);
		}
		options = options || {};
		return this.each(function () {
			...
			if (state) {
				...
				options = $.extend({}, $.fn.desktop.defaults, $.fn.desktop.parseOptions, options);
				$.data(this, 'desktop', {
					options : options
				});
			}
	...
	$.fn.desktop.methods = {
		options : function () {
			return this.data().desktop.options;
		},
		appcontainer : function () {
			return this.data().desktop.appContainer;
		},
		calendar : function () {
			return this.data().desktop.calendar;
		},
		start : function () {
			return this.data().desktop.start;
		},
		taskbar : function () {
			return this.data().desktop.taskBar;
		},
		tasklist : function () {
			return this.data().desktop.taskList;
		},
		startmenu : function () {
			return this.data().desktop.start.data().menu;
		},
		layout : function () {
		...
		seticonsize : function (size) {
			return this.each(function () {
				$.data(this, 'desktop').options.iconSize = size;
				var appContainer = $.data(this, 'desktop').appContainer;
	...
	$.fn.desktop.parseOptions = function () {};
